...
function generateFindings(c, e, m, f, s) {
  ...
  if (!c.ownerRenounced && (c.hasOwnerFn || c.owner)) add('OWNER_ACTIVE', 'low', 'Ownership not renounced', 'Owner can change contract state.', '#');
  if (!e || !e.verified) add('UNVERIFIED_SOURCE', 'medium', 'Contract source not verified', 'Source code has not been verified on Blockscout.', '#');
  ...
  if (m) {
    ...
    if (m.liquidityUsd < 1000) add('VERY_LOW_LIQUIDITY', 'high', 'Liquidity is extremely low', `< $1K liquidity.`, '#');
      ...
      add('LOW_LIQUIDITY', 'medium', 'Low liquidity to valuation ratio', 'Pool liquidity is less than 1% of FDV.', '#');
    }
  ...
  // Funding
  if (f) {
    if (f.funding_parent_share >= 0.5) add('SHARED_FUNDING_PARENT', 'high', 'Shared funding wallet', `${pct(f.funding_parent_share)} of buyers share funder.`, '#');
    else if (f.funding_parent_share >= 0.2) add('COORDINATED_BUYING', 'medium', 'Coordinated buying detected', `${pct(f.funding_parent_share)} share funder.`, '#');
    ...
    if (f.cluster_dominance >= 0.7) add('CLUSTER_DOMINANCE', 'high', 'Wallet cluster dominance', `${pct(f.cluster_dominance)} clustered.`, '#');
      ...
      add('COORDINATED_BUYING', 'medium', 'Coordinated buying detected', `${pct(f.cluster_dominance)} clustered.`, '#');
    }
    ...
    if (f.fresh_wallet_ratio != null && f.fresh_wallet_ratio >= 0.5) add('FRESH_WALLETS_RATIO', 'medium', 'High fresh wallet ratio', `${pct(f.fresh_wallet_ratio)} fresh wallets.`, '#');
    if (f.size_cv != null && f.size_cv < 0.15 && f.buyersAnalyzed >= 5) add('UNIFORM_BUY_SIZES', 'medium', 'Uniform buy sizes', 'Low variance buy sizes.', '#');
    ...
    if (sub === 'Coordinated' && !findings.some(x => x.code === 'COORDINATED_BUYING' || x.code === 'SHARED_FUNDING_PARENT' || x.code === 'CLUSTER_DOMINANCE')) {
      add('COORDINATED_BUYING', 'medium', 'Coordinated buying detected', 'Early buyers show coordinated activity.', '#');
    } else if (sub === 'Extraction' && !findings.some(x => x.code === 'SHARED_FUNDING_PARENT' || x.code === 'CLUSTER_DOMINANCE')) {
      add('CLUSTER_DOMINANCE', 'high', 'Wallet cluster dominance', 'Early buyers show extraction cluster dominance.', '#');
    }
  }
